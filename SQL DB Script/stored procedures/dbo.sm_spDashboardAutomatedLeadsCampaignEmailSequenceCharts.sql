CREATE OR ALTER PROCEDURE [dbo].[sm_spDashboardAutomatedLeadsCampaignEmailSequenceCharts] 
    @startDate DATETIME, 
    @endDate DATETIME,
    @bdr VARCHAR(500),
...
BEGIN   
	...
	SET @baseFilter = '
		...
		AND CONVERT(DATE, sle.SentTime) BETWEEN @startDate AND @endDate
	';
	...
	BEGIN
		...
		WITH DailyBreakdown AS (
			SELECT 
				CONVERT(VARCHAR(10), CAST(sle.SentTime AS DATE), 23) AS ExportedDate,
				ISNULL(NULLIF(sla.BDR, ''''), ''Others'') AS LeadUsers,
				COUNT(sla.Id) AS TotalCount
			' + @baseFilter + '
			AND sla.BDR = @bdr
			GROUP BY CAST(sle.SentTime AS DATE), sla.BDR
		)
	...
	BEGIN
		...
		WITH DailyBreakdown AS (
			...
			GROUP BY CAST(sle.SentTime AS DATE), sla.CreatedBy
		)
		SELECT ExportedDate, LeadUsers, TotalCount
		FROM DailyBreakdown
		ORDER BY ExportedDate DESC, CASE WHEN LeadUsers = ''Others'' THEN 1 ELSE 0 END, LeadUsers
	...
	BEGIN
		...
		WITH DailyBreakdown AS (
			...
			' + @baseFilter + '
			AND sla.QABy = @qaby
			GROUP BY CAST(sle.SentTime AS DATE), sla.QABy
		)
		SELECT ExportedDate, LeadUsers, TotalCount
		FROM DailyBreakdown
		ORDER BY ExportedDate DESC, CASE WHEN LeadUsers = ''Others'' THEN 1 ELSE 0 END, LeadUsers
