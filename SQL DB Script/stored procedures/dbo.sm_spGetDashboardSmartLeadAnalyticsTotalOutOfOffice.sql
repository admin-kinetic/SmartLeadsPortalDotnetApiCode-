CREATE OR ALTER PROCEDURE [dbo].[sm_spGetDashboardSmartLeadAnalyticsTotalOutOfOffice]
@startDate DATETIME,
@endDate DATETIME,
@bdr VARCHAR(500),
...
BEGIN   
	SET NOCOUNT ON;
	SELECT COUNT(DISTINCT sla.Email) AS TotalOutOfOffice from SmartLeadAllLeads sla
	INNER JOIN SmartLeadsEmailStatistics ses ON sla.Email = ses.LeadEmail
	WHERE (sla.SmartleadCategory IS NOT NULL AND sla.SmartleadCategory = 'Out Of Office')
