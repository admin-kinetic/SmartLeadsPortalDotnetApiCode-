CREATE OR ALTER PROCEDURE [dbo].[sm_spGetDashboardSmartLeadAnalyticsTotalUniqueOpened]
@startDate DATETIME,
@endDate DATETIME,
@bdr VARCHAR(500),
...
BEGIN   
	SET NOCOUNT ON;
	SELECT COUNT(DISTINCT sla.Email) AS TotalUniqueOpened from SmartLeadAllLeads sla
	INNER JOIN SmartLeadsEmailStatistics ses ON sla.Email = ses.LeadEmail
	WHERE (ses.OpenTime IS NOT NULL OR ses.OpenTime <> '')
	...
	AND (
	    ...
	    OR (CONVERT(DATE, ses.OpenTime) >= CONVERT(DATE, @startDate) 
	        AND CONVERT(DATE, ses.OpenTime) <= CONVERT(DATE, @endDate))
	)
END
GO