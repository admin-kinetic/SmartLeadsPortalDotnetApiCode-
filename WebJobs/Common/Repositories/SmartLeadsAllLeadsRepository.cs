...
public class SmartLeadsAllLeadsRepository
{
    ...
    {
        logger.LogInformation("Start UpsertLeadFromEmailSent");
        ...
        try
        {
            ...
            logger.LogInformation("UpsertLeadFromEmailSent took {ElapsedMilliseconds} ms", stopwatch.ElapsedMilliseconds);
        }
        catch (Exception ex)
        {
            await transaction.RollbackAsync();
            logger.LogError(ex, "Error on UpsertLeadFromEmailSent");
            throw;
        }
    }
    ...
    {
        logger.LogInformation("Start UpsertLeadFromEmailLinkClick");
        var stopwatch = System.Diagnostics.Stopwatch.StartNew();
        ...
        try
        {
            ...
            await connection.ExecuteAsync(upsert, lead, transaction);
            await transaction.CommitAsync();
            logger.LogInformation("UpsertLeadFromEmailLinkClick took {ElapsedMilliseconds} ms", stopwatch.ElapsedMilliseconds);
        }
        catch (Exception ex)
        {
            await transaction.RollbackAsync();
            logger.LogError(ex, "Error on UpsertLeadFromEmailLinkClick");
            throw;
        }
    }
    ...
    public async Task UpdateLeadCategory(string email, string category)
    {
        try
        {
            var lead = await GetByEmail(email);
            if (lead == null)
            {
